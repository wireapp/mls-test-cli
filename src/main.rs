...
async fn new_key_package(backend: &impl OpenMlsCryptoProvider) -> KeyPackageBundle {
    ...
    future::block_on(async move {
        match cli.command {
            ...
            Command::KeyPackage {
                ...
                let key_package_bundle = new_key_package(&backend).await;
            ...
            Command::Group {
                ...
                let kp_bundle = new_key_package(&backend).await;
                let kp = kp_bundle.key_package();
                let kp_ref = kp.hash_ref(backend.crypto()).unwrap();
                let kp_hash = kp_ref.value();

                let mut group =
                    MlsGroup::new(&backend, &group_config, group_id, kp_hash.as_slice())
                        .await
                        .unwrap();
